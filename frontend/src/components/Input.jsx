...
const Input = ({ convStart, setConvStart }) => {
	...
	const { ref: registerMessageRef, ...registerMessageProps } = register("message", { required: false });
	...
	return (
		<div className="flex flex-col gap-[4rem]">
			...
			{/* Form */}
			<form className="px-5 flex flex-col items-center justify-center" onSubmit={handleSubmit(onSubmit)}>
				<div className="border min-h-[2rem] px-2 py-2 rounded-[10px] flex flex-col-reverse sm:flex-col w-full max-w-full md:max-w-2/3 lg:max-w-1/2 bg-[#fff]">
					...
					<div className="flex items-center justify-between w-full text-xl gap-3">
						<div className="flex gap-2 items-center">
							...
							{showComponent && <Fileupload file={file} setFile={setFile} promt={message} />}
						</div>
						<div className="flex items-center justify-center gap-3">
							<i className="ri-mic-line"></i>
							<i className="ri-send-plane-fill cursor-pointer" onClick={() => handleSubmit(onSubmit)()}></i>
						</div>
					</div>
