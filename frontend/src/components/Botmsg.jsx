  ...
  return (
    <div className="p-3 my-2 bg-gray-900 text-white rounded-lg shadow max-w-full space-y-4">
      ...
      {msg?.charts?.map((chart, idx) => {
        ...
        const series = [
          { name: chart.seriesName || "Value", data: chart.data.map((d) => d.value) },
        ];
        const meta = chart.meta || {}; // optional meta info (stock, weather, sales etc.)

        return (
          <div
            key={idx}
            className="w-full bg-gray-800 rounded-xl p-4 shadow-md space-y-3"
          >
            {/* Optional Header */}
            {meta.title && (
              <div>
                ...
                {meta.subtext && (
                  <div className="text-gray-400 text-sm">{meta.subtext}</div>
                )}
              </div>
            )}
